...
0008: 3A 00 88        LD      A,(watchdog)        ; Feed the watchdog
000B: 31 00 88        LD      SP,$8800            ; Stack at top of RAM
000E: C3 A3 02        JP      Initialize          ; 
...
0032: 10 F4           DJNZ    PrintString         ; Do all B-count characters
0034: C9              RET                         
                  
0035: FF FF FF 
Clear Screen
There are 32 rows of 32 columns. This clears the screen from right to left one column at a time with a noticeable delay between columns.
...
006A: DD E5           PUSH    IX                  
006C: FD E5           PUSH    IY                  
006E: 3A 00 88        LD      A,(watchdog)        ; 
0071: AF              XOR     A                   
0072: 32 08 B8        LD      (IRQENABLE),A       ; Disable interrupts
...
00C8: 20 04           JR      NZ,$CE              ; 
00CA: AF              XOR     A                   
00CB: 32 18 B8        LD      (COINCNT0),A        ; 
00CE: 3A 04 E0        LD      A,(INPUTS_C)        ; 
00D1: E6 08           AND     $08                 
...
024E: E1              POP     HL                  
024F: 3E 01           LD      A,$01               
0251: 32 08 B8        LD      (IRQENABLE),A       ; Enable interrupts
0254: F1              POP     AF                  
0255: ED 45           RETN                        
...
Initialize:  
02A3: AF              XOR     A                   
02A4: 32 08 B8        LD      (IRQENABLE),A       ; Disable interrupts
02A7: 32 05 88        LD      ($8805),A           
02AA: 32 10 B8        LD      (FLIPX),A           ; 
02AD: 32 0C B8        LD      (FLIPY),A           ; 
...
02C3: 2C              INC     L                   
02C4: 10 FC           DJNZ    $2C2                ; 
02C6: 3A 02 E0        LD      A,(INPUTS_B)        ; 
02C9: 16 2E           LD      D,$2E               
02CB: E6 03           AND     $03                 
02CD: 5F              LD      E,A                 
02CE: 1A              LD      A,(DE)              
02CF: 32 E4 83        LD      ($83E4),A           
02D2: 3A 04 E0        LD      A,(INPUTS_C)        ; 
02D5: 67              LD      H,A                 
02D6: CB 5C           BIT     3,H                 
02D8: 28 05           JR      Z,$2DF              ; 
02DA: 3E 01           LD      A,$01               
...
02F8: 32 08 B8        LD      (IRQENABLE),A       ; Enable interrupts
02FB: FF              RST     $38                 ; Clear the screen
02FC: AF              XOR     A                   
02FD: 32 01 B0        LD      (SPRITERAM+1),A     ; 
0300: 3E 06           LD      A,$06               
0302: 32 03 B0        LD      (SPRITERAM+3),A     ; 
...
0327: 32 D9 83        LD      (soundCtrl),A       ; ... of sound control
032A: 32 02 D0        LD      (SOUND_CTRL),A      ; De-assert interrupt, set amp AM/PM bit
032D: AF              XOR     A                   ; Send turn-off-all-sounds to ...
032E: CD 94 07        CALL    SendSoundCmd        ; ... the sound board
0331: 3A D9 83        LD      A,(soundCtrl)       ; Clear ...
0334: E6 EF           AND     $EF                 ; ... Amplifier AM/PM ...
0336: 32 D9 83        LD      (soundCtrl),A       ; ... bit
0339: 32 02 D0        LD      (SOUND_CTRL),A      ; Set new value
033C: 3E FF           LD      A,$FF               ; Send reset command to ...
033E: CD 94 07        CALL    SendSoundCmd        ; ... the sound board
...
05A8: 67              LD      H,A                 
05A9: 6F              LD      L,A                 
05AA: 32 10 B8        LD      (FLIPX),A           ; 
05AD: 32 0C B8        LD      (FLIPY),A           ; 
...
SendSoundCmd:
0794: 32 00 D0        LD      (SOUND_CMD),A       ; Write sound command
0797: 3A D9 83        LD      A,(soundCtrl)       ; State of port B
079A: E6 F7           AND     $F7                 ; Lower the ...
079C: 32 02 D0        LD      (SOUND_CTRL),A      ; ... sound interrupt bit
079F: 00              NOP                         ; Brief ..
07A0: 00              NOP                         ; ...
07A1: 00              NOP                         ; ...
07A2: 00              NOP                         ; ... Pause
07A3: 3A D9 83        LD      A,(soundCtrl)       ; State of port B
07A6: F6 08           OR      $08                 ; Raise the ...
07A8: 32 02 D0        LD      (SOUND_CTRL),A      ; ... sound interrupt bit
07AB: C9              RET                         ; Done
...
07B4: 2C              INC     L                   
07B5: 7E              LD      A,(HL)              
07B6: CD 94 07        CALL    SendSoundCmd        ; Send command to the sound board
07B9: 54              LD      D,H                 
07BA: 5D              LD      E,L                 
07BB: 2C              INC     L                   
07BC: 06 00           LD      B,$00               
...
0853: 67              LD      H,A                 
0854: 32 10 B8        LD      (FLIPX),A           ; 
0857: 32 0C B8        LD      (FLIPY),A           ; 
085A: C9              RET                         
...
1B74: 3A 02 E0        LD      A,(INPUTS_B)        ; 
1B77: 4F              LD      C,A                 
1B78: C3 FB 1A        JP      $1AFB               ; 

1B7B: 3A 04 E0        LD      A,(INPUTS_C)        ; 
1B7E: CB 47           BIT     0,A                 ; ?? 2P Down
1B80: C3 15 1B        JP      $1B15               ; 

1B83: 3A 00 E0        LD      A,(INPUTS_A)        ; 
...
2CF5: 3A 00 E0        LD      A,(INPUTS_A)        ; Coins and Service Mode
...
2D03: CD 94 07        CALL    SendSoundCmd        ; Send coin-inserted to sound board
2D06: AF              XOR     A                   
