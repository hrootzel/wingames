...
export function activateSpecialsOnce(board, activeSet = null) {
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      let rr = r - 1;
      while (rr >= 0 && board[rr][c] === CELL_EMPTY) rr--;

      if (rr >= 0 && isCoin(board[rr][c])) {
  ...
  while (guard++ < 64) {
    ...
    for (const group of toConvert) {
      ...
      if (group.tier < 5) {
        ...
        spawns.push({ r: anchor[0], c: anchor[1], cell: cellOfTier(group.tier + 1) });
      }
    ...
    for (const spawn of spawns) {
      let r = spawn.r;
      if (r < 0 || r >= ROWS || board[r][spawn.c] !== CELL_EMPTY) r = findTopFreeRow(board, spawn.c);
      if (r < 0) continue;
