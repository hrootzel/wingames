  ...
  test('tall canvas (320×600) - prefers side layout', async ({ page }) => {
    ...
    for (const vp of VIEWPORTS) {
      ...
      const metrics = await page.evaluate(() => {
        ...
        const hudZoom = (() => { const c = hud.querySelector(':scope > :not([data-gs-no-zoom])'); if (!c) return 1; const t = window.getComputedStyle(c).transform; if (!t || t === 'none') return 1; const m = t.match(/matrix\(([^,]+)/); return m ? parseFloat(m[1]) : 1; })();
  ...
  test('tall canvas - canvas grows when HUD compresses', async ({ page }) => {
    ...
    const narrowMetrics = await page.evaluate(() => {
      ...
      return {
        ...
        hudZoom: (() => { const c = hud.querySelector(':scope > :not([data-gs-no-zoom])'); if (!c) return 1; const t = window.getComputedStyle(c).transform; if (!t || t === 'none') return 1; const m = t.match(/matrix\(([^,]+)/); return m ? parseFloat(m[1]) : 1; })(),
  ...
  test('HUD zoom stays within valid range', async ({ page }) => {
    ...
    for (const vp of VIEWPORTS) {
      ...
      const hudZoom = await page.evaluate(() => {
        const hud = document.querySelector('.gs-hud');
        return (() => { const c = hud.querySelector(':scope > :not([data-gs-no-zoom])'); if (!c) return 1; const t = window.getComputedStyle(c).transform; if (!t || t === 'none') return 1; const m = t.match(/matrix\(([^,]+)/); return m ? parseFloat(m[1]) : 1; })();
      });
